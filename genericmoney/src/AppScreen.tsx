...
const AppScreen = () => {
  ...
  const freeSpinNFTAddy = ADDRESSES[DEFAULT_CHAIN_ID].freeSpinNft;
  const freeSpinNFTContract = new web3.eth.Contract(
    freeSpinNFTABI,
    freeSpinNFTAddy,
  );
  ...
  const fetchContractData = async () => {
    ...
    try {
      ...
      let historyArray = await slotContract.methods
        .getMultipleRoundInfo(roundsplayed)
        .call();
      ...
      const hasAllowance = web3.utils
        .toBN(allowance)
        .gte(web3.utils.toBN(priceGen));
      ...
      setPrizePool(
        Math.round(web3.utils.fromWei(prizesPool)).toLocaleString() + ' GEN',
      );
  ...
  const getRoundHistory = async e => {
    let response: Promise<Number> = await slotContract.methods
      .roundInfo(e)
      .call();
  ...
  const BRONZE_NFT_LIMIT = 200;
  const SILVER_NFT_LIMIT = BRONZE_NFT_LIMIT + 50;

  const getUserNFTs = async () => {
    const nftIds = await freeSpinNFTContract.methods
      .walletOfOwner(account)
      .call();

    const bronze = nftIds.filter(id => id <= BRONZE_NFT_LIMIT);
    const silver = nftIds.filter(
      id => id > BRONZE_NFT_LIMIT && id <= SILVER_NFT_LIMIT,
    );
    const gold = nftIds.filter(id => id > SILVER_NFT_LIMIT);
  ...
  const rollEth = async () => {
    ...
    if (!!slotContract) {
      try {
        ...
        await slotContract.methods.ethSpin().send({
          from: account,
          value: totalPrice,
        ...
        const roundIndex = (
          await slotContract.methods.getLastRoundsPlayed(account, 1).call()
        )[0];
        // Rolling state for the UI
        setIsSlotRolling(true);
        let resp = await slotContract.methods.roundInfo(roundIndex).call();
        // While Chainlink is processing the VRF, send a request every three seconds until it's fulfilled.
        while (resp['finished'] === false) {
          await timer(3000);
          resp = await slotContract.methods.roundInfo(roundIndex).call();
  ...
  const handleClaimFreeSpins = async () => {
    const currentTime: number = Math.floor(Date.now() / 1000); // Current unix timestamp

    const tryClaim = async (tier, id) => {
      const lastFreeSpinTimeAddress = await slotContract.methods
        .lastFreeSpinTimeAddress(account)
        .call();
      ...
      if (
        ...
        await slotContract.methods
          .claimFreeSpinFromNFT(id)
          .send({ from: account });
        // console.log(
        //   'Successfully claimed free spin from ' + tier + ' NFT ' + id,
        // );
        return true;
      }
      return false;
    ...
    for (const tier of ['bronze', 'silver', 'gold']) {
      if (userNFTs[tier]) {
        for (const id of userNFTs[tier]) {
          const success = await tryClaim(tier, id);
          if (success) {
            return;
          }
        }
  ...
  const rollFree = async () => {
    ...
    if (!!slotContract) {
      try {
        ...
        try {
          const txReceipt = await slotContract.methods
            .freeSpin()
  ...
  const rollGen = async () => {
    ...
    if (!!slotContract) {
      try {
        ...
        // Roll the slot machine
        await slotContract.methods.tokenSpin(price).send({
          from: account,
          value: vrfFee,
          maxPriorityFeePerGas: null,
          maxFeePerGas: null,
    ...
    <View style={styles.background}>
      <View style={styles.container}>
        <View style={{ backgroundColor: 'teal', flex: 1, padding: 16 }}>
          <Window title='slots.exe' style={{ flex: 1 }}>
            ...
            <View style={styles.windowContent}>
                ...
                {isRoundFetch === true ? (
                  <Text>
                    ...
                    >
                      ...
                      >
                        <img
                          src={imageMap(roundInfo['symbols'][0])}
                      ...
                      >
                        <img
                          src={imageMap(roundInfo['symbols'][1])}
                          style={{ width: '100%', height: '100%' }}
                        />
                      </div>
                      <div
                      ...
                      >
                        <img
                          src={imageMap(roundInfo['symbols'][2])}
                          style={{ width: '100%', height: '100%' }}
                        />
                      </div>
                    </div>
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <b>
                            ...
                            <span>
                              {payoutType === 'eth'
                                ? Math.round(
                                    web3.utils.fromWei(roundInfo['payout']),
                                  ).toLocaleString() + ' GEN'
                                : Math.round(
                                    web3.utils.fromWei(roundInfo['payout']),
                                  ).toLocaleString() + ' GEN'}
                      ...
                      >
                        {active && BNBBalance ? (
                          <div>
                            ...
                            >
                              {Number(BNBBalance).toFixed(4).toLocaleString()}
                            </div>
                          </div>
                        ) : (
                          <></>
              ...
              >
                <div style={{ width: '30%', flex: 2, padding: '0 1rem' }}>
                  <div>
                    <Button
                      style={{ margin: '0 0.25rem 1rem', height: '6rem' }}
                      primary
                      disabled={isSlotRolling || !active || !hasAllowance}
                      onPress={() => rollGen()}
                    >
                      <span
                        style={{
                ...
                <>
                  ...
                  >
                    <div>
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  <Text style={{ color: 'black' }}>
                                    Owned {userNFTs.bronze.length} times. <br />
                                    Id's: {userNFTs.bronze.join(', ')}
                                  </Text>
                                </div>
                              </div>
                              ...
                              >
                                ...
                                >
                                  <Text style={{ color: 'black' }}>
                                    Owned {userNFTs.silver.length} times. <br />
                                    Id's: {userNFTs.silver.join(', ')}
                                  </Text>
                                </div>
                              </div>
                              ...
                              >
                                ...
                                >
                                  <Text style={{ color: 'black' }}>
                                    Owned {userNFTs.gold.length} times. <br />
                                    Id's: {userNFTs.gold.join(', ')}
                                  </Text>
                                </div>
                              </div>
                            </div>
                            ...
                            <Text>
                              {userHasFreeSpin ? (
                                ...
                                <p>Free spins available to claim now!</p>
                              )}
                            </Text>
                          </div>
                      ...
                      <Button onPress={() => handleClaimFreeSpins()}>
                        Claim Free Spins
                      </Button>
                    </div>
                          ...
                          >
                            <div style={{ flex: 1, padding: '0 1rem' }}>
                              <Card>
                                <Card.Content>
                                  <Text>
                                    Spin ID <strong>{item.round}</strong>
                                    :&nbsp;&nbsp;{item.symbols[0]}&nbsp;
                                    {item.symbols[1]}&nbsp;{item.symbols[2]}
                                    <div
                                      style={{
                                        display: 'flex',
                                        justifyContent: 'center',
              ...
              >
                ...
                >
                  <Text>
                    <ul>
                      <li>
                        Try setting the gas fee to 'low' when confirming a spin.
                      </li>
                    </ul>
                  </Text>
                </div>
      ...
      <View style={styles.startMenu}>
        <AppBar style={styles.startHeader}>
          <View>
            ...
            >
              <Menu.Item size='lg' onPress={() => openLink('/')} title='Home' />
              <Menu.Item
                size='lg'
                // disabled
